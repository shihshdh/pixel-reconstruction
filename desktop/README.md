...
用 [Tauri 2](https://tauri.app) 打包的桌面客户端。窗口里加载的是线上站点 <https://pixel-reconstruction.netlify.app>，所以站点更新后客户端不需要重新安装。
...
安装包输出到 `src-tauri/target/release/bundle/nsis/`。换图标时替换 `app-icon.png`（1024×1024），再运行 `npm run icons`。
...
- 站点可以通过 `window.__PIXEL_DESKTOP__` 判断自己是否运行在客户端里。客户端只向站点开放两个本地命令：往作品文件夹写文件（文件名和类型都有白名单，不能写到别的目录）、打开作品文件夹。`info.json` 不包含任务 Token 或下载链接。
