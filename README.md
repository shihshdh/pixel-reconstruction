...
一张照片 → Apple SHARP 高斯泼溅 → 浏览器里的 3D 场景 + 云端渲染的运镜视频。
...
没有要配的第三方 token——只有 Modal 一个云服务。

总共五步，照着做就行。每一步末尾都有"看到什么算成功"。
...
Modal 免费额度每月 30 美元（以官网为准）。A10G 约 $1.1/小时，
一张图占用 GPU 约 30~60 秒，折合人民币几分钱。Vercel 免费档足够。
也就是说：**额度内这个站完全零成本。**

## 之后可以加的玩法

服务端运镜目前用的是 SHARP 自带的默认环绕轨迹；想要"缓推成片"
"希区柯克变焦成片"，下一步可以自己写 gsplat 渲染脚本接管相机轨迹。
另外还有：.ply 转 .ksplat 压缩加速加载、给接口加个口令防止被刷、
生成历史画廊。哪个想做了，回来找 Claude 继续。
