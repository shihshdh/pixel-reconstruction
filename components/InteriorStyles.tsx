...
export default function InteriorStyles() {
  return <style>{`
  :root{--bg:#f6f4ee;--bg2:#efeee6;--card:#fbfaf5;--track:#e9e7dc;--ink:#252820;--ink2:#646958;--ink3:#737765;--line:#dfdfd2;--line2:#cecebc;--accent:#77784e;--glass:rgba(249,248,241,.68);--glass-brd:rgba(255,255,245,.85);--nav-brd:#5d644014;--shadow:0 24px 64px -32px #25291b59;}
  [data-theme="dark"]{--bg:#1b1e19;--bg2:#22261f;--card:#292d25;--track:#34382e;--ink:#edede1;--ink2:#b0b2a1;--ink3:#a0a68f;--line:#aeb39923;--line2:#bec3a134;--accent:#a5ad7c;--glass:rgba(38,43,34,.76);--glass-brd:#eef3d824;--nav-brd:#dce4c214;--shadow:0 24px 64px -32px #000a;}
  .ruhua-root{--surface-tint:#fffff94f;--surface-rim:#fffffccc;--surface-shadow:#444b2810;--blue:var(--accent);--violet:#92946c;--pink:#aa977d;background:radial-gradient(ellipse at 75% 0%,#b3b98c15,transparent 50%),var(--bg);font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif;}
  [data-theme="dark"].ruhua-root{--surface-tint:#d1d6b80b;--surface-rim:#edf0d727;--surface-shadow:#0003;}
  .ruhua-root ::selection{background:#afb78a55;}
  .ruhua-root{overflow-x:clip;}
  .landing-fallback{position:fixed;inset:0;z-index:10000;overflow:hidden;background:#181914;color:#fbf9f0;font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif;}
  ...
  .landing-fallback>p{position:absolute;left:5.2vw;bottom:36px;font-size:10px;letter-spacing:.08em;margin:0;color:#f1f1e5ae;}
  ...
  .showcase-shutter{position:absolute;left:-4%;width:108%;height:20.2%;background:var(--bg);border-bottom:1px solid var(--line2);box-shadow:0 -4px 28px #26311f17;transform:translate3d(105%,0,0);animation:showcase-shutter-enter 780ms cubic-bezier(.65,0,.25,1) both;}
  .showcase-shutter:nth-child(even){background:var(--bg2);}
  .showcase-shutter span{display:block;height:100%;background:linear-gradient(108deg,transparent 45%,#fffff35c 70%,transparent);}
  ...
  .home-hero::before{background:radial-gradient(ellipse at 60% 10%,#c4c7a328,transparent 68%);}
  ...
  .nav-drop{position:absolute;left:0;top:15px;width:100px;height:35px;border-radius:99px;transform-origin:left;pointer-events:none;transition:transform 420ms cubic-bezier(.16,1,.3,1);background:linear-gradient(160deg,#ffffffa8,#ffffff12 45%,#a5ad7833 77%,#ffffff87);box-shadow:inset 1px 1px 1px #ffffffe0,inset -1px -1px 1px #a4ae7b6b,inset 0 0 8px #c6cf9d24,0 3px 8px #505a2813;border:1px solid #a5ad7827;backdrop-filter:blur(2px) saturate(150%);-webkit-backdrop-filter:blur(2px) saturate(150%);}
  ...
  [data-theme="dark"] .nav-drop{background:linear-gradient(160deg,#e9efd526,#e9efd505 50%,#bac5971a);box-shadow:inset 1px 1px 1px #eef4d95c,inset -1px -1px 1px #b4be811e,0 3px 8px #0002;border-color:#d9e2b823;}
  .nav-theme{position:relative;overflow:hidden;background:linear-gradient(140deg,#ffffff80,#ffffff08 55%,#c3cca72e)!important;box-shadow:inset 1px 1px 1px #ffffffe0,inset -1px -1px 1px #9fab6a42!important;}
  ...
  @media(hover:hover){.ruhua-root button:not(:disabled):not(.nav-logo):not(.nav-tab):hover{transform:translateY(-3px);box-shadow:0 7px 18px -10px #3039228c;}.ruhua-root .nav-tab:hover{transform:translateY(-1px);}.ruhua-root .seg-btn:hover{transform:translateY(-1px)!important;box-shadow:none!important;}}
  ...
  .ruhua-root :focus-visible{outline:2px solid #929969;outline-offset:5px;}
  ...
  .liquid-surface::before,.compute-panel::before{background:radial-gradient(ellipse at var(--pointer-x,15%) var(--pointer-y,0%),#ffffff48,transparent 62%),linear-gradient(170deg,#ffffeb0b,transparent 50%,#96976b08);}
  .liquid-surface::after,.compute-panel::after{background:linear-gradient(145deg,var(--surface-rim),transparent 30%,#bbc19c18 60%,var(--surface-rim));}
  .liquid-thumb{background:linear-gradient(155deg,#b6bd9199,#898e63e8 42%,#747b50eb 70%,#a9b186aa);border-color:#edf0d58f;box-shadow:inset 1px 1px 1px #f8f9dc9c,inset -1px -1px 2px #50593463,0 3px 10px #49502726;}
  .liquid-toggle{isolation:isolate;background:linear-gradient(150deg,#ffffff24,#b0b89a14 60%,#ffffff40)!important;box-shadow:inset 0 1px 0 #ffffff8c,inset 0 -1px 0 #7d8b4421,0 7px 18px -12px #3d452952;backdrop-filter:blur(4px) saturate(145%)!important;-webkit-backdrop-filter:blur(4px) saturate(145%)!important;}
  .liquid-toggle[data-tone="dark"]{background:linear-gradient(155deg,#26312470,#141d1545)!important;box-shadow:inset 0 1px 1px #ffffff36,inset 0 -1px 0 #ffffff10,0 7px 18px -12px #0008;}
  .liquid-toggle .liquid-thumb,.liquid-toggle .dock-thumb{overflow:hidden;background:linear-gradient(158deg,#ffffffc7,#e7ecd380 44%,#bfcc967d 72%,#ffffefab);border:1px solid #ffffff70;box-shadow:inset 1px 1px 1px #fffffff5,inset -1px -1px 2px #74815477,inset 0 0 5px #ffffed66,0 5px 10px -5px #3c462047;backdrop-filter:blur(3px) saturate(165%);-webkit-backdrop-filter:blur(3px) saturate(165%);}
  .liquid-toggle .liquid-thumb::after,.liquid-toggle .dock-thumb::after{mix-blend-mode:normal;background:radial-gradient(ellipse at var(--pointer-x,18%) var(--pointer-y,0%),#ffffffc9,transparent 65%),linear-gradient(176deg,#ffffff66,transparent 40%,#b6c18a20 80%,#ffffef80);transition:opacity 180ms;opacity:.76;}
  ...
  .liquid-toggle[data-tone="dark"] .seg-btn[aria-pressed="true"]{color:#262d1f!important;text-shadow:0 1px 0 #ffffff66;}
  [data-theme="dark"] .liquid-toggle[data-tone="light"] .seg-btn[aria-pressed="true"]{color:#262d1f!important;}
  .source-drop{background:linear-gradient(150deg,#ffffefbf,#fffff512 50%,#aab18514);border-color:var(--surface-rim);}
  ...
  .service-status.is-ready i{background:#83916a;box-shadow:0 0 0 4px #929c7512;}
  ...
  .dz{min-height:330px!important;border-radius:13px!important;background:linear-gradient(140deg,#bfc4a514,var(--card))!important;padding:42px!important;}
  ...
  .dz-over .dz-arrow{box-shadow:0 14px 30px -14px #65733b80;}
  ...
  .develop-stage{background:linear-gradient(145deg,#d9dccc0c,var(--bg2));border-radius:15px;border-color:var(--line);padding:28px 22px;}
  .develop-mat{padding:13px;background:linear-gradient(140deg,#c2bda5,#777863 25%,#c3bea7 60%,#6f745a);border-radius:2px;box-shadow:inset 0 0 0 2px #f6f3db20,0 20px 40px -25px #262d2299;}
  ...
  .status-orb{width:5px;height:5px;box-shadow:0 0 0 4px #929c7512;}
  .gallery-card{border-radius:14px;box-shadow:0 12px 26px -22px #2d342769;}
  .gallery-actions button:first-child{background:var(--ink);color:var(--bg);}
  .gallery-cache-label.local,.gallery-save-status.local{color:#7c8a58;}
  .gallery-empty{border-radius:15px;padding:64px 24px;background:#bbc1a20a;}
  .gallery-empty button{background:var(--ink);color:var(--bg);}
  .doubao-settings{border-radius:15px;}
  .doubao-emblem{border-radius:11px;background:#a6af7614;}
  .prompt-picker{border-radius:15px!important;}
  .prompt-tile{border-radius:10px!important;background:var(--glass)!important;}
  .prompt-tile[aria-pressed=true]{background:#a4ae7820!important;}
  .prompt-preview summary{color:var(--accent);}
  .loop-fill{background:linear-gradient(90deg,#727c4e,#bdc6a0);}
  ...
  .motion-section{background:#1b2119!important;}
  .motion-section>div{filter:sepia(.15);}
  .closing-section::before{background:radial-gradient(ellipse,#b8bf941b,transparent 68%);}
  ...
  .home-page>section:nth-child(2)>div:first-child{border-radius:15px!important;box-shadow:0 24px 40px -30px #24311d66!important;}
  ...
  .workflow-step[aria-pressed="true"]{background:linear-gradient(100deg,#a8b58212,#a8b58203);}
  .workflow-step[aria-pressed="true"]>i{opacity:1;transform:none;}
  .workflow-preview{margin:24px 0 0;max-width:370px;}
  .workflow-preview-scene{position:relative;aspect-ratio:360/220;overflow:hidden;border:1px solid var(--line2);border-radius:10px;background:linear-gradient(140deg,#a2ad8822,#d0d7b71c 50%,#bcc7a738);perspective:650px;isolation:isolate;}
  .workflow-preview-grid{position:absolute;inset:-25%;opacity:0;background-image:linear-gradient(#79895120 1px,transparent 1px),linear-gradient(90deg,#79895120 1px,transparent 1px);background-size:24px 24px;transform:rotateX(50deg) rotateZ(-12deg) translateY(40px);transition:opacity 400ms;}
  .workflow-preview-plane{position:absolute;inset:17px 49px 29px;border:1px solid #f8f6e76e;box-shadow:0 16px 26px -17px #303c249e;transform:translate3d(0,0,0);transition:transform 550ms cubic-bezier(.16,1,.3,1),box-shadow 450ms;}
  ...
  .workflow-preview[data-stage="1"] .workflow-preview-plane{transform:translate3d(-8px,-3px,0) rotateY(-18deg) rotateX(7deg);box-shadow:14px 17px 0 -3px #7e8c5921,0 18px 28px -14px #303c2455;}
  ...
  .workflow-camera-path{position:absolute;inset:0;width:100%;height:100%;color:#d6e2b2;opacity:0;transition:opacity 300ms;filter:drop-shadow(0 1px 2px #1f2d1d66);}
  .workflow-preview[data-stage="2"] .workflow-camera-path{opacity:1;}
  .workflow-path-shadow{stroke:#35432a8c;stroke-width:5;}
  ...
  .workflow-preview-controls button[aria-pressed="true"]{color:var(--ink);background:#a2ae7621;box-shadow:inset 0 1px 1px var(--surface-rim);}
  ...
  .compute-story figure{margin:0;position:relative;overflow:hidden;border-radius:12px;background:#181e16;}
  .compute-story img{width:100%;height:195px;object-fit:contain;display:block;filter:saturate(.65);}
  .compute-story figcaption{position:absolute;left:14px;bottom:12px;font-size:8px;letter-spacing:.1em;color:#d4d8bd8c;}
